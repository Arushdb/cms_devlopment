...
export class AwardBlankSheetComponent implements OnInit, OnDestroy {
  ...
  employeeCourseHttpServiceResultHandler(res) {
    ...
    this.awardsheet_params = this.awardsheet_params.set(
      'time',
      new Date().toString()
    );
    ...
    if (String(employeeCourse + '').length == 0) {
      ...
      for (var obj of employeeCourse.CodeList.root) {
        ...
        employeeCourseArrCol.push({
          ...
          semesterStartDate: obj.semesterStartDate,
          semesterEndDate: obj.semesterEndDate,
          courseCode: obj.courseCode,
          courseName: obj.courseName,
          programCourseKey: obj.programCourseKey,
          resultSystem: obj.resultSystem,
          ...
          gradelimit: obj.gradelimit,
          createrId: obj.creatorId,
          Marks: '',
        });
  ...
  gradeHttpService() {
    ...
    this.subs.add = this.userservice
      .getdata(this.awardsheet_params, obj)
      .subscribe((res) => {
        //this.userservice.log(" in switch detail selected");
        res = JSON.parse(res);
  ...
  getupdatedgradelimitForSaveResultHandler(res) {
    ...
    if (isUndefined(res.CodeList.root)) {
      ...
      for (var obj of res.CodeList.root) {
        this.gradelimitarraycoll.push({
          coursecode: obj.courseCode,
          grades: obj.grades,
          marksfrom: obj.marksfrom,
          marksto: obj.marksto,
        });
      }
    }
  ...
  furtherExecution(event): void {
    ...
    this.awardsheet_params = this.awardsheet_params.set(
      'specCode',
      event.data.specializationId
    );
    this.awardsheet_params = this.awardsheet_params.set(
      'semesterCode',
      event.data.semesterCode
    );
    this.awardsheet_params = this.awardsheet_params.set(
      'semesterName',
      event.data.semesterCode
    ...
    this.awardsheet_params = this.awardsheet_params.set(
      'branchName',
      event.data.branchName
    );
    this.awardsheet_params = this.awardsheet_params.set(
      'spclName',
      event.data.specializationName
    );
  ...
  resultHandlerComponent(res) {
    ...
    if (!isUndefined(res.ComponentList.component)) {
      for (let object of res.ComponentList.component) {
        this.componentAC.push({
          evaluationId: object.evaluationId,
          evaluationIdName: object.evaluationIdName,
          group: object.group,
          rule: object.rule,
          idType: object.idType,
          displayType: object.displayType,
          maximumMarks: object.maximumMarks,
    ...
    this.subs.add = this.userservice
      ...
      .subscribe(
        (res) => {
          //this.userservice.log(" in switch detail selected");
          res = JSON.parse(res);
          if (mode == 'getmarks') this.getStudentMarks();

          if (mode == 'submitsheet') this.submitSheet();
        },
        (error) => {
          this.userservice.log(
            'There is some issue at server ,Please try again'
          );
          this.spinnerstatus = false;
        }
      );
  ...
  resultHandlerStudentMarks(res) {
    ...
    for (var obj1 of this.studentXml) {
      ...
      if (isUndefined(res.MarksList.marks)) {
        ...
        for (var obj2 of res.MarksList.marks) {
          if (
            String(obj1.rollNumber).toString() ===
            String(obj2.rollNumber).toString()
          ) {
            ...
            // no marks record present in Student Marks
          }
        }
      }
    ...
    console.log(
      ...
      this.gradesCalculated,
      'submitstatusofotherteacher',
      this.submitstatusofotherteacher,
      'this.gradeauthorityholder',
      this.gradeauthorityholder,
      'allowEdit:',
      this.allowEdit,
    ...
    if (this.courseType == 'Reg') this.setNewColumnsmk();
    else this.setcoreColumnsmk();
    this.spinnerstatus = false;
  }
  ...
  setColumnsmk() {
    ...
    this.componentAC.forEach((column: any) => {
      let definition: ColDef = {
        headerName:
          column.evaluationIdName +
          '/' +
          column.maximumMarks +
          '/' +
          column.group,
        field: column.evaluationId,
        width: 125,
      };
  ...
  getgroups(): any[] {
    ...
    for (let i = 0; i < this.componentAC.length; i++) {
      ...
      if (
        ...
        String(this.componentAC[i].group).toString()
      ) {
        continue;
      } else {
  ...
  resultHandlerStatus(res) {
    ...
    if (this.courseType == 'Reg') this.getEvaluationComponents();
    else this.getcoreEvaluationComponents();
  }
  ...
  hasValuesetter = function (params: ValueSetterParams): boolean {
    ...
    if (isNaN(Number(params.colDef.cellRendererParams.values[0]))) {
      this.userservice.log(
        params.column.getColId() + ':Component Max marks invalid'
      );
    ...
    if (
      ...
      // if value is undefined or null or blank
      params.data[id] = 'Invalid';
      return false;
    }
  };
  ...
  httpSavecell(refreshgrid) {
    ...
    this.subs.add = this.userservice
      .postdata(this.awardsheet_params, obj)
      .subscribe((res) => {
        if (refreshgrid) {
          this.getStudentMarks();
        }
  ...
  httpGetgetCourseAuthorityDetails(res) {
    ...
    if (!isUndefined(res.CodeList.root)) arrayCollect = res.CodeList.root;
    ...
    if (arrayCollect.length === 0) {
      ...
      console.log(
        'creatorId',
        arrayCollect[0].creatorId,
        'employeeId',
        arrayCollect[0].employeeId
      );

      this.authorityHolderId = String(arrayCollect[0].employeeId).toString();

      if (
        String(arrayCollect[0].creatorId).toString() ===
        String(arrayCollect[0].employeeId).toString()
      ) {
        ...
        this.userservice.log(
          arrayCollect[0].employeeName +
            ' has authority to enter grades for ' +
            this.awardsheet_params.get('courseCode') +
            ' for ' +
            this.awardsheet_params.get('semesterstartdt') +
            ' / ' +
            this.awardsheet_params.get('semesterenddt')
        );
      }
    }
  ...
  onclickgradelimitbutton() {
    ...
    dialogConfig.data = {
      ...
      menuType: 'AwardSheet',
      creatorEntity: this.awardsheet_params.get('entityId'),
    ...
    this.subs.add = dialogRef.afterClosed().subscribe((result) => {
      if (result) this.getStudentMarks();
    });
  }
  ...
  getCourseGradeLimitSuccess(res) {
    ...
    if (!isUndefined(res.courseDetails)) {
      ...
      for (let i = 0; i < gradelimitstatus.length; i++) {
        ...
        if (String(gradelimitstatus[i].userId).toString() === 'Declared') {
        ...
        if (!this.gradeauthorityholder) {
          ...
          if (
            ...
            if (
              teacherary.includes(String(this.authorityHolderId).toString())
            ) {
    ...
    if (
      ...
      this.allowEdit = 'N';
      this.userservice.log(
        'Grades can only be entered after all other sheets are submitted'
      );
    }
    ...
    if (resultprocessed) {
      this.allowEdit = 'N';
      this.userservice.log(
        'Result is already Processed ,Grading can not be changed '
      );
    }
  ...
  onFileChange(event: any) {
    ...
    if (file) {
      ...
      if (isUndefined(fname)) {
        console.log('Arush File name undefined');
        this.userservice.log('Please select correct file');
        this.spinnerstatus = false;
        event.target.value = '';
        return;
      }
      ...
      if (
        ...
        filematched = true;
        console.log('Arush File name matched');
      }
      if (!filematched) {
        this.userservice.log('Please select valid file');
        this.spinnerstatus = false;
        event.target.value = '';
        return;
      }
      ...
      reader.onload = (e: any) => {
        ...
        let result: any;
        this.gridOptionsmk.api?.forEachNode((node) => {
          console.log(node.data.rollNumber);
          result = jsonData.find(
            (rec: any) =>
              String(rec.rollNumber).toString() ==
              String(node.data.rollNumber).toString()
          );
          if (result) {
            console.log(result);
            let evid = '';
          ...
          } else console.log(node.data.rollNumber, 'not found'); // { id: 2, name: "Bob", age: 30 }
        });
      };
      //this.spinnerstatus = false;
    ...
    this.gridOptionsmk.api?.redrawRows();
    this.gridOptionsmk.api?.refreshCells({ force: true });
  ...
  exportToExcel() {
    ...
    let rowData: any = [];
    let copiedrowData: RowNode[] = [];
    //const rowDataCopy = structuredClone(this.gridOptionsmk.api?.getr);
    // copiedrowData=this.gridOptionsmk.api?.getRenderedNodes()||[];
    this.gridOptionsmk.api?.forEachNode((node) => {
      copiedrowData.push(node);
    });

    debugger;
    //const copiedData = this.rowData.map(row => ({ ...row }));

    let evid = '';
    let evidname = '';

    // this.gridOptionsmk.api?.forEachNode((node) => {
    copiedrowData.forEach((node) => {
      //console.log(node.data.E02[0]);
      console.log(node.data);
      node.data.studentName = node.data.studentName[0];
    ...
    //     column.evaluationIdName[0];
    //   });

    //   // rowDataforexcel.push(consoleelement)
    // });
    // console.log(rowData); // Array of all row data
