...
export class LoginformComponent implements OnInit {
  ...
  studentlogin() {
    ...
    this.subs.add = this.userservice.getdata(this.reg_params, obj).subscribe(
      (res) => {
  ...
  displaystudent(studentdata) {
    ...
    this.dialogRefreg = this.dialog.open(
  ...
  getRegistrationDeadlines(res) {
    let obj = { xmltojs: 'Y', method: 'None' };
    this.reg_params = this.reg_params.set('userName', this.f.username.value);
    this.reg_params = this.reg_params.set(
      'program_id',
      String(res.studentdata.student[0].program_id[0]).trim()
    );
    this.reg_params = this.reg_params.set(
      'branch_code',
      String(res.studentdata.student[0].branch_code[0]).trim()
    );
    this.reg_params = this.reg_params.set(
      'new_specialization',
      String(res.studentdata.student[0].new_specialization[0]).trim()
    );
    this.reg_params = this.reg_params.set(
      'program_id',
      String(res.studentdata.student[0].program_id[0]).trim()
    ...
    this.reg_params = this.reg_params.set(
      'semester_code',
      String(res.studentdata.student[0].semester_code[0]).trim()
    );
    this.reg_params = this.reg_params.set(
      'entity_id',
      String(res.studentdata.student[0].entity_id[0]).trim()
    );
    let studentdata = res;
    ...
    this.subs.add = this.userservice.getdata(this.reg_params, obj).subscribe(
      (res) => {
        ...
        let regenddate = String(
          result.Deadlines.deadlines[0].registration_end_date[0]
        )
          .trim()
          .slice(0, 10);
        let regstartdate = String(
          result.Deadlines.deadlines[0].registration_start_date[0]
        )
          .trim()
          .slice(0, 10);
      ...
      (error) => {
        ...
        if (error.originalError.status == 501)
          message =
            'Registration dead lines have not been set up.Please contact EdRP';
